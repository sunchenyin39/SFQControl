...
def main():
    ...
    parameters.Nc = 44  # subsequence所包含的SFQ时钟周期数
    parameters.Nq = 8  # subsequence所包含的量子比特进动周期数
    parameters.omegaSFQ = 2*np.pi*25E9  # SFQ时钟频率
    parameters.anharmonicity = 2*np.pi*250E6  # 非简谐角频率的绝对值
    parameters.times = 7  # subsequence重复的次数
    ...
    parameters.itenumber = 20000  # GA算法的繁衍次数
    parameters.power = 30  # GA算法的复制函数参数 power<popsiza/2
    parameters.pc = 0.1  # GA算法的交叉概率
    parameters.pm = 1  # GA算法的变异概率
    parameters.mutnumber = 30  # 单次变异基因数量
    ...
    # Subsequence=[]
    # print(SFQControl.quantum.Fedelity(SFQControl.quantum.UGgenerator(parameters.USFQ, parameters.UFR, Subsequence, parameters.times),parameters.matrix))
