...
def main():
    ...
    parameters.USFQ_P = SFQControl.quantum.USFQgenerator(parameters.deltatheta)  # 单个正SFQ脉冲的演化算符
    parameters.USFQ_N = SFQControl.quantum.USFQgenerator(-parameters.deltatheta)  # 单个负SFQ脉冲的演化算符
    parameters.UFR = SFQControl.quantum.UFRgenerator(parameters.omegaq, parameters.T_SFQclock, alpha)  # 单个SFQ时钟周期内量子比特的自由演化算符
    parameters.popsize = 100  # GA算法的种群数量
    parameters.itenumber = 10000  # GA算法的繁衍次数
    parameters.power = 30  # GA算法的复制函数参数 power<popsiza/2
    parameters.pc = 0.6  # GA算法的交叉概率
    parameters.pm = 0.15  # GA算法的变异概率
    parameters.mutnumber = 5  # 单次变异基因数量
    ...
    SFQControl.GA.GA_bipolar(parameters)
